...
export default class EventManager extends Singleton {
  ...
  public static dispatch(event: EventEnum, ...params: unknown[]){
    this.Instance.emit(event, ...params);
  }

  public static addListener(event: EventEnum, cb: (...params: unknown[]) => void, ctx: unknown = null) {
    this.Instance.on(event, cb, ctx);
  }
}
